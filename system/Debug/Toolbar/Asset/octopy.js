...
function newXHR() {
	...
	realXHR.addEventListener("readystatechange", function() {
		...
		if (realXHR.readyState === 4 && realXHR.status.toString()[0] === '2' && realXHR.responseURL.indexOf('toolbar_time') === -1) {
			var toolbarTime = realXHR.getResponseHeader('Debugbar-Time');
			if (toolbarTime) {
				var h2 = document.querySelector('#octopy-history > h2');
				h2.innerHTML = 'History <small>You have new debug data.</small> <button onclick="loadDoc(' + toolbarTime + ')">Update</button>';
				var badge = document.querySelector('a[data-tab="octopy-history"] > span > .badge');
